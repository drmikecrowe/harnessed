...
class TestGitleaksMigrated:
    """gitleaks: Go binary from a pinned release tarball; PreToolUse hook guard script.

    The old shape had three mechanisms (mise aqua binary, Dockerfile COPY for the guard, setup:
    nag for host hooks) — none of which worked on --host. This recipe is ONE install.sh run by
    both executors (harnessed-2jk).
    """

    def test_declares_install_with_pinned_cache_and_no_dockerfile(self):
        r = load_recipe(CATALOG / "recipes" / "gitleaks", strict=True)
        assert r.install is not None
        assert r.install.script == "install.sh"
        assert r.install.cache == "v8.30.1"
        assert r.install.system is None  # binary lands in $HARNESSED_BIN_DIR — nothing needs root
        assert not (r.root / "Dockerfile").exists(), "no Dockerfile: the binary is fetched by install.sh"

    def test_scripts_pinned_version_matches_cache_key(self):
        """install.cache in recipe.yaml and GITLEAKS_VERSION in install.sh must agree.

        A drifted pair keys the host cache by one version while the script downloads another —
        the cache dir holds stale bytes forever, silently.
        """
        r = load_recipe(CATALOG / "recipes" / "gitleaks", strict=True)
        body = (r.root / r.install.script).read_text(encoding="utf-8")
        assert 'GITLEAKS_VERSION="8.30.1"' in body, (
            "GITLEAKS_VERSION in install.sh must match install.cache ('v8.30.1') in recipe.yaml"
        )

    def test_passes_install_lint(self):
        validate_install_script(load_recipe(CATALOG / "recipes" / "gitleaks", strict=True))

    def test_guard_script_is_present(self):
        r = load_recipe(CATALOG / "recipes" / "gitleaks", strict=True)
        assert (r.root / "gitleaks-guard").is_file(), "gitleaks-guard script must be in the recipe dir"

    def test_hook_is_wired_as_pretooluse_bash(self):
        """The recipe must declare a PreToolUse/Bash hook so the assembler writes it into
        settings.json. Without this, the guard is on PATH but never called."""
        r = load_recipe(CATALOG / "recipes" / "gitleaks", strict=True)
        assert r.hooks, "recipe must declare at least one hook"
        pre = r.hooks.get("PreToolUse", [])
        assert any(
            getattr(h, "matcher", None) == "Bash" for h in pre
        ), "PreToolUse must have a Bash-matcher entry (the gitleaks-guard gate)"
