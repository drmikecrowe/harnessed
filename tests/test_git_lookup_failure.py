...
class TestFailuresRaise:
    def _explode(self, monkeypatch, exc):
        monkeypatch.setattr(paths.subprocess, "run", lambda *a, **k: (_ for _ in ()).throw(exc))
    ...
    def test_an_unrecognised_git_error_raises(self, repo, monkeypatch):
        ...
        self._explode(
            monkeypatch,
            subprocess.CalledProcessError(128, "git", stderr="fatal: detected dubious ownership"),
        )
        with pytest.raises(paths.GitLookupFailed):
            paths.git_common_dir_checked(repo)
...
class TestTheLossyWrapperIsUnchanged:
    ...
    def test_it_swallows_a_failure(self, repo, monkeypatch):
        monkeypatch.setattr(
            paths.subprocess,
            "run",
            lambda *a, **k: (_ for _ in ()).throw(subprocess.TimeoutExpired(cmd="git", timeout=5)),
        )
...
class TestProjectEnvPathRefusesToGuess:
    def test_it_propagates_the_failure(self, repo, monkeypatch):
        monkeypatch.setattr(
            paths,
            "git_common_dir_checked",
            lambda _p: (_ for _ in ()).throw(paths.GitLookupFailed("boom")),
        )
        with pytest.raises(paths.GitLookupFailed):
