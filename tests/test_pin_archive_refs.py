...
class TestGithubArchiveRefs:
    ...
    def test_a_branch_archive_is_rejected(self, ref):
        with pytest.raises(PinValidationError) as exc:
            validate_pin(
                "r", _dockerfile(f"curl -fsSL https://github.com/o/r/archive/{ref}.tar.gz | tar xz")
            )
        assert ref in str(exc.value), "the error must name the offending ref"

    def test_a_sha_archive_passes(self):
        validate_pin(
            "r", _dockerfile(f"curl -fsSL https://github.com/o/r/archive/{SHA}.tar.gz | tar xz")
        )
    ...
    def test_a_zip_archive_is_gated_too(self):
        with pytest.raises(PinValidationError):
            validate_pin(
                "r", _dockerfile("curl -fsSL https://github.com/o/r/archive/main.zip -o s.zip")
            )
...
class TestCodeloadRefs:
    ...
    def test_a_branch_is_rejected(self, kind):
        with pytest.raises(PinValidationError):
            validate_pin(
                "r", _dockerfile(f"curl -fsSL https://codeload.github.com/o/r/{kind}/main -o s")
            )

    def test_a_sha_passes(self):
        validate_pin(
            "r", _dockerfile(f"curl -fsSL https://codeload.github.com/o/r/tarball/{SHA} -o s")
        )
...
class TestVariableRefs:
    ...
    def test_an_unassigned_variable_fails_closed(self, tmp_path):
        ...
        r = _script_recipe(
            tmp_path, ('curl -fsSL "https://github.com/o/r/archive/$MYSTERY.tar.gz" -o s.tgz\n')
        )
        with pytest.raises(PinValidationError) as exc:
            validate_install_script(r)
    ...
    def test_a_positional_parameter_is_a_pass_through(self):
        ...
        validate_pin(
            "r", _dockerfile('curl -fsSL "https://github.com/$1/archive/$2.tar.gz" -o "$3/src.tgz"')
        )
