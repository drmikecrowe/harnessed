...
auth never persists across recreates. Current behavior (HRN-04) only emits a warning in
`lib/harnessed-isolated-config.sh` (the `antigravity` branch of
...
- On launch (harness-aware, antigravity only), start `dbus-launch` + `gnome-keyring-daemon
  ...
  attach (`lib/harnessed-isolated.sh` attach branches + likely a helper in
  `lib/harnessed-isolated-config.sh`).
- Persist the keyring store dir (`~/.local/share/keyrings`) to
  `$XDG_STATE_HOME/harnessed/<project>/<stack>/` — mirror the existing `.claude`
  state-persistence/copy-on-start pattern in `harnessed-isolated.sh` (wipe only on first
  create or `--fresh`), so a one-time OAuth survives normal recreates.
