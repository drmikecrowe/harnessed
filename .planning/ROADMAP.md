...
Plans:
- [ ] 04-01: Shared service sidecars (image/volume/lifecycle) + `svc up/down/list` + concurrent attach over harnessed-net
- [ ] 04-02: State persistence + `--fresh` + full CLI (`list`/`stop`/`rm`/`new`/`install`/`uninstall` shims)
...
|-------|----------------|--------|-----------|
| 1. Containerized Engine + Transparent Stack | 0/3 | Not started | - |
