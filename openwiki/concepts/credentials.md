---
type: "Reference"
title: "Credential handling: enter, resolve, mount, never bake"
...
tags: [credentials, secrets, varlock, secrets-broker, topology-b, ssh-agent, isolated-auth, aws-sso, corp-proxy-ca, build-secret, sop]
...
# Credential handling: enter, resolve, mount, never bake
...
The SOP is now also a **backend contract clause**. `seed_auth` is one of the six capabilities every `ExecutionBackend` implements (`backend.py`), and its docstring carries the rule verbatim: give the harness the host's credentials *by reference* — mount or symlink, never a copy — and **a backend that cannot reference a live store must fail rather than snapshot one**. Sequencing is deliberately backend-owned (there is no fixed-order driver), so where `seed_auth` sits in a launch differs per backend; what may never change is that it references.
...
- **Lifecycle.** On the container backend the broker is started inside `apply_isolation(BOUNDARY)` — **before** `pod create`, because the pod's network args depend on whether one exists — and is stopped if that create fails (Ctrl-C included: a broker that outlives its launch is a host process holding live secrets nothing will ever reap by name). `_pod_teardown` stops it **first** and never fatally — the pod is the containment boundary. `broker.reconcile`, run by `harnessed list` before reporting, reaps every recorded broker whose pod is gone: the backstop for every teardown path that never ran.
...
The stack field `forward_git_credentials` (default **off**) gates the secret-bearing slice: the gh oauth token file and opt-in private SSH keys. But the **SSH agent socket + read-only git identity config are forwarded automatically whenever the agent socket is live on the host**, independent of that flag. The asymmetry is deliberate: an agent socket exposes no key material and gates every sign/auth behind a host-side 1Password approval or YubiKey touch, so "1Password available → wired up" is safe as a default — while anything secret-bearing requires the stack to ask. (`forward_aws_sso` is opt-in for the same reason: STS creds pulled over the ECS endpoint are *not* touch-gated.)

## Per-harness auth paths

### Where the container path composes credentials: `ContainerBackend.seed_auth`

On the container backend, credential composition is the `seed_auth` contract operation on `ContainerBackend` (`launcher.py`). Its body does exactly two things, in this order:

1. **Resolve launch secrets** — `_resolve_launch_secrets(spec.project_path)`, layered global → project, producing the ordered `--env-file` list and its generated temps. This deliberately runs **after** every aborting check in `materialize_config` (unsupported harness, unreachable aws-sso endpoint, and the rest), so an early exit can never strand resolved secrets in temp files on disk.
2. **Append the Claude credential mount LAST.** Whether a credential file is mounted at all depends on a `CLAUDE_CODE_OAUTH_TOKEN` that may only arrive via `--env-file`, so the decision cannot be made until resolution has happened — and the mount is appended after everything else composes.

The resolved lists are stored on the backend instance and consumed downstream by `apply_isolation(BOUNDARY)`, which is the phase that actually delivers mounts and env across the pod boundary: the `--env-file` args are passed to the one `podman run` **unconditionally**, and the temp files are unlinked in a `finally` around that run — success or failure — because resolved secrets must not linger on disk (T-05-06). The full sequencer order is `provision_tools(FIRST_START)` → `materialize_config` → `seed_auth` → `wire_mcp` → `apply_isolation(BOUNDARY)` → CA install → `provision_tools(ATTACH)` → `apply_isolation(EGRESS)`.

```mermaid
flowchart TD
    PT["provision_tools FIRST_START - compose volumes"]
    MC["materialize_config - compose the mount set, with aborting checks"]
    SA["seed_auth - resolve env-files, then append the Claude credential mount LAST"]
    WM["wire_mcp"]
    AI["apply_isolation BOUNDARY - pod create delivers mounts and env"]
    ENV["env-file temps passed unconditionally, then unlinked in a finally"]
    CAI["install corp proxy CA"]
    ATT["provision_tools ATTACH - setup scripts"]
    EG["apply_isolation EGRESS - close the firewall"]
    PT --> MC --> SA --> WM --> AI --> ENV --> CAI --> ATT --> EG
```

*The container sequencer: seed_auth resolves after every aborting check and the mount lands last; the temp env-files die in the same `finally` that starts the container.*

One known imprecision is tracked in source (harnessed-0tk.1.1): several *other* credential mounts (the onboarding stub, the mcp-auth store, the omp agent dir, the git/SSH forwards) are composed in `materialize_config` rather than in `seed_auth`. They are emitted as one ordered block today, and regrouping podman `-v` arguments would be an unverifiable change to the one path the test suite never executes. The host backend's `seed_auth` is correspondingly thin: it delegates to the harness's `HostHarness.share_state` row (claude's `.credentials.json` + session dirs, omp's `agent.db` + shared state), run inside the home lock immediately after materialization.
...
- **`CLAUDE_CODE_OAUTH_TOKEN` is primary** (mechanism 2): a long-lived subscription token from `claude setup-token`, roughly a one-year lifetime. It never expires mid-session, needs no in-container refresh, and therefore makes the whole copy-divergence problem unnecessary. When one is configured, **no credential file is mounted at all** — `_claude_creds_seed_mount` returns empty and `seed_auth` appends nothing. A token-free `~/.claude.json` onboarding stub is mounted separately in all cases so the agent skips first-run setup — the stub is written to a per-instance state dir and never carries the token.
...
- **`isolated_auth: true` (stack) answers a different question** — not "how does the container get your login" but "how does this stack run as *someone else's*". The host token is withheld from *both* supply routes. The `-e` forward is suppressed at the `podman run` composition, and `_strip_var_from_env_files` deletes the variable from the generated env-file temps in place — necessary because `--env-file` is passed unconditionally, so a token declared in the *user-global* `.env.schema` (the normal way to hold your own token) would walk straight past the forward suppression and arrive anyway. Stripping is preferred over injecting `-e VAR=` because the latter would depend on the harness reading an empty token as "absent", which is unverified; absent is unambiguous. No host credential is seeded, and the stack gets a per-instance credentials file **minted in-container by its own `/login`**, seeded as `{}` so it starts logged out, persisted on the host across recreates and cleared by `--fresh`. This is **not a fourth SOP violation, because nothing is copied**: the store is the only copy of that credential in existence, so there is no second copy to diverge from and no refresh race. It lives **outside the `~/.claude` config volume** on purpose: `_ensure_config_volume` destroys that volume whenever the profile fingerprint changes, under an explicit "safe to destroy: credentials are bind-mounted over it and live on the host" invariant that this mount relies on — a login stored in the volume would be wiped by the next recipe edit. **The flag is claude-only by gating, not by accident**: `seed_auth` checks `isolated_auth and harness == "claude"`, and an `isolated_auth` stack on another harness prints a warning and falls through to the normal path — omp authenticates from the very `CLAUDE_CODE_OAUTH_TOKEN` this branch would strip, so suppressing it there would leave an omp launch with no auth at all while the warning promised the opposite. The field's own comment records the simpler alternative: a `CLAUDE_CODE_OAUTH_TOKEN` the client hands you outright in `<project>/.env.schema` needs no file, no expiry, and nothing to log into — `isolated_auth` is for the case where the client wants you to log in interactively as them. The same identity split propagates to the mcp-remote OAuth token store (`~/.mcp-auth` for host-identity stacks, a per-instance dir for isolated ones), mounted rw so a consent outlives the pod.
...
- **aws-sso forwarding is a broker URL, not a copy.** For `forward_aws_sso: true` stacks, `materialize_config` calls `_aws_sso_ecs_forward_args`, which injects exactly two `-e` values and nothing else: `AWS_CONTAINER_CREDENTIALS_FULL_URI` (the host's aws-sso ECS server via `host.containers.internal`) and `AWS_CONTAINER_AUTHORIZATION_TOKEN` (its bearer, read from the user-owned token file `harnessed aws-sso serve` writes, mode 0600, in the config dir). The in-container AWS SDK pulls short-lived STS credentials over HTTP; no aws-sso binary, store, or SSO token ever enters the container, and the bearer arrives as a per-launch `-e`, never an image layer. When the token file exists but the server itself is unreachable (never started this session, or no role loaded — probed via the unauthenticated `/healthcheck`), the launch warns and asks to confirm continuing without AWS credentials; under `container-exec`/headless there is nobody to ask, so the confirm is skipped and the launch exits 1 — the designed non-interactive answer, rather than parking a supervised launch on a prompt that would hang it. Wiring the dead endpoint anyway would only fail later, when the SDK first calls AWS and the failure looks like a broken token. A token file that is *absent* means this host never set AWS SSO up, so the flag stays a silent no-op — which is what makes `forward_aws_sso` safe to commit in a shared catalog.
...
Two other pieces complete the picture. `--fresh` performs explicit wipes of the persisted in-container auth stores (antigravity's keyring, and an isolated-auth stack's own login) because both deliberately survive ordinary recreates, so "start clean" requires the removal to be spelled out rather than inherited from teardown. And the container backend's `seed_auth` ordering, described above, is itself a disk-hygiene measure: resolution runs only after every aborting check, and the temp env-files are unlinked in a `finally` around the podman run either way.
...
- The isolated-auth store must stay outside the config volume, and `_ensure_config_volume`'s "safe to destroy" invariant depends on credentials never living inside it. Its claude-only gating is load-bearing too: stripping the token variable for a non-claude harness would leave that harness with no auth at all.
- `seed_auth` must keep resolving after the aborting checks and appending the credential mount last, and `_strip_var_from_env_files` must keep running against the same generated temps the `podman run` consumes — resolving earlier strands secrets on an aborted launch; dropping the strip lets a user-global token walk past isolation through the unconditional `--env-file`.
...
- [Backends](/openwiki/architecture/backends.md) — the `ExecutionBackend` contract `seed_auth` belongs to, and why sequencing is backend-owned.
...
- [State](/openwiki/architecture/state.md) — where the per-instance credential stores, broker records, and token files live on disk.
