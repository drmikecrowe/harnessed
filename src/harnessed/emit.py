...
def write_derived_dockerfile(
    ...
    - If `stack.hatago.repo` is set, appends a layer reinstalling hatago itself (`pnpm add -g
      <repo>#<ref>`) on top of the base image's pinned release — for testing a personal fork.
      Requires a pinned `hatago.ref` (validate_hatago_repo_pin, called from assemble() before
      this runs).
    """
    lines: list[str] = [
    ...
    hatago_repo = stack.hatago.get("repo")
    if hatago_repo:
        git_spec = shlex.quote(f"{hatago_repo}#{stack.hatago['ref']}")
        lines += [
            "# --- hatago override (stack hatago.repo) ---",
