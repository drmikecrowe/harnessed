...
# A DEFAULTLESS `ARG KEY` line (no `=`) — e.g. the agent Dockerfiles' `ARG OMP_VERSION`, whose value
# comes from agent.yaml's build_args. write_derived_dockerfile re-emits those with their value
# inlined, so it must drop the original bare declaration (a bare `ARG KEY` AFTER `ARG KEY=v` resets
# the value to empty).
_BARE_ARG_RE = re.compile(r'^ARG\s+([A-Za-z_][A-Za-z0-9_]*)\s*$', re.IGNORECASE)


def _bare_arg_key(line: str) -> str | None:
    """The KEY of a defaultless `ARG KEY` line, else None."""
    m = _BARE_ARG_RE.match(line.strip())
...
def write_derived_dockerfile(
    ...
    *, hatago: dict | None = None, with_scan: bool = True,
    agent_dockerfile: Path | None = None, agent_build_args: dict[str, str] | None = None,
) -> Path:
    ...
    - Uses FROM harnessed-base:latest.
    ...
    - Appends the AGENT's Dockerfile body LAST (agent-last lineage), so the harness CLI installs
      ON TOP of the recipe layers rather than underneath them.

    Why agent-last: the agent CLI pins (e.g. OMP_VERSION) churn far faster than the recipes. When
    the agent image was the FROM parent, every agent bump changed the parent image id and so
    invalidated EVERY recipe layer of every stack on that harness. With the agent on top, an agent
    bump invalidates only the agent layer + the scan, and the (expensive) recipe layers stay cached.

    It also makes the recipe layers harness-independent: they now hang off harnessed-base with
    identical instructions for every harness, so a stack declaring `harnesses: [claude, omp]` builds
    its recipe layers ONCE and both harnesses share them. (The one recipe that references ${HARNESS}
    in a RUN — ccstatusline — necessarily splits the cache from its own layer onward; that is the
    cost of the ARG, and it is why HARNESS-dependent recipes should sort late.)
    """
    lines: list[str] = [
    ...
    # --- agent LAST (see docstring) ---
    # The agent's own Dockerfile is `FROM harnessed-base` + a couple of RUN layers (install the
    # harness CLI). Strip its FROM (we are already in a stage) and inline its body here. Its
    # `ARG <KEY>` declarations carry no default (agent.yaml's build_args are the single source of
    # truth), so they are replaced with `ARG KEY=<value>` — otherwise a bare re-declaration would
    # reset the value to empty and the pin would silently vanish.
    if agent_dockerfile is not None and agent_dockerfile.is_file():
        build_args = agent_build_args or {}
        body = [
            ln for ln in agent_dockerfile.read_text(encoding="utf-8").splitlines()
            if not ln.strip().upper().startswith("FROM ")
            and not _ARG_HARNESS_RE.match(ln.strip())
            and _bare_arg_key(ln) not in build_args
        ]
        lines.append(f"# --- agent: {harness} (installed last so agent pins don't bust recipe layers) ---")
        # Restore the USER the agent Dockerfiles were written against. They declare no USER of their
        # own: as `FROM harnessed-base` they used to inherit `harnessed` (the base's final USER)
        # directly. Now that recipe layers sit in between, a recipe that ends as root would silently
        # install the harness CLI into /root. Every recipe happens to end `USER harnessed` today —
        # this makes that an invariant of the build rather than a convention recipes must remember.
        lines.append("USER harnessed")
        lines.extend(f"ARG {key}={val}" for key, val in build_args.items())
        lines.extend(body)
        lines.append("")
