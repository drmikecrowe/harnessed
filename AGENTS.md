...
how-to guides are under [docs/guides/](docs/guides). Two generated maps exist and they answer
different questions: `openwiki/` covers BEHAVIOR — precedence, launch order, invariants — with
checkable per-claim evidence (see §Generated wiki below), while
...
The managed block below is deliberately passive — it only says the wiki exists. This section is the
active guidance, and it sits OUTSIDE the markers so an `openwiki` run cannot rewrite it.

| Need | Go to |
| --- | --- |
| Who wins when two config sources conflict | `openwiki/concepts/precedence.md` — one row per conflict, naming the winner and the bug the loser produced |
| Launch order; what differs between the two backends | `openwiki/workflows/container-run.md`, `openwiki/workflows/host-run.md` |
| What a gate proves, and what it does not | `openwiki/testing/verification-ladder.md` |
...
`docs/codebase/CONCERNS.md` and `CONVENTIONS.md` — do not delete those two expecting the wiki to
have absorbed them.
