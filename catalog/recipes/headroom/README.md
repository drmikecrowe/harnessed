...
2. Port 8787 already bound → reused, never double-spawned. Free → `headroom proxy --port 8787`
   starts in the background (log: `/tmp/headroom-proxy.log`).
...
unmanaged daemon: it dies with the pod / host session and restarts on the next launch.
