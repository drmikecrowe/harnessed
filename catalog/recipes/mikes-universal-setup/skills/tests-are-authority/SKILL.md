---
name: tests-are-authority
description: An existing test encodes a decision already made; your change does not outrank it. Load when a test fails after your change, before editing an existing test, and before adding skip/xfail or loosening an assertion.
---
...
- Never delete a test you do not understand. Same reflex the `load-bearing-comments` skill exists to
  stop.
...
- "It passes without it" never justifies deleting a check — load the `load-bearing-comments` skill,
