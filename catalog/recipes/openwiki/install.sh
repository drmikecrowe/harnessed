...
# BEFORE the `rm -rf` below, deliberately. openwiki declares `engines.node >= 22` and its CLI dies
# at import on an older runtime, so an unmet requirement must be caught while the previous working
# install is still on disk. Discovered in review: the destructive step used to run first, so a host
# launch on old node destroyed a good tree and then failed.
node_major="$(node -p 'process.versions.node.split(".")[0]' 2>/dev/null || echo 0)"
if [[ "$node_major" -lt 22 ]]; then
    echo "error: install (openwiki) needs Node 22 or newer (found major '${node_major}')." \
         "openwiki declares engines.node >=22 and fails at import below it." >&2
    exit 1
fi
...
# The MCP entry point, and the reason it is a wrapper rather than `openwiki mcp --host <literal>`.
#
# `--host` is the AUTHORING AGENT's identity, not the launcher's: session-manager.js:52 does
# `producerActor = options.producerActor ?? options.host` and records `metadataModel:
# host-agent/<host>`, so the value becomes run metadata and generated provenance. The agent doing
# the writing is claude, omp, codex … — never harnessed, which starts the pod and authors nothing.
#
# A literal in `recipe.yaml` would therefore stamp every harness with one agent's name, and
# `${HARNESS}` cannot go in `args:` either: emit.py writes stdio `args` VERBATIM and substitutes
# `${VAR}` only for a `url_env` URL, so the placeholder would reach openwiki intact and fail its
# `isValidHostId` check. `$HARNESS` is in the folder-env contract and reaches this child through
# hatago's inherited container env, so resolving it at spawn time is the one place that works.
#
# `openwiki integrations install` knows ids for claude, codex and opencode only. omp and
# antigravity are absent there, but `isValidHostId` accepts any lowercase name, so those record
# their own harness rather than someone else's. Honest for all five with no branching.
cat > "$HARNESSED_BIN_DIR/openwiki-mcp-host" <<'WRAPPER'
#!/usr/bin/env bash
# Generated by catalog/recipes/openwiki/install.sh — do not edit.
set -euo pipefail
exec openwiki mcp --host "${HARNESS:-unknown}" "$@"
WRAPPER
chmod +x "$HARNESSED_BIN_DIR/openwiki-mcp-host"
