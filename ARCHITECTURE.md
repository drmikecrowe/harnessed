...
exits non-zero if registration fails. `run --create-aoe-only` still mints and builds — the row
replays `harnessed launch`, which hard-errors without an assembled profile. Skipping the build
creates a row that is dead on arrival.
